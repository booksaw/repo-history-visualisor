...
export default function angleMaximisation(links: LinkData[], id: (node: NodeData) => string, velocityMultiplier: number = 0.1, angleAllowance: number = 0.1): AngleMaximisation<NodeData> {
    let nodes: NodeData[];
    ...
    function updateNodeData(node: NodeData) {
        ...
        outgoingNodes.forEach((outgoing) => {
            ...
            // const targetVectorOrigin = Vector.add(targetVector, nodeVector);
            outgoing.vx = requiredChange.x * velocityMultiplier;
            outgoing.vy = requiredChange.y * velocityMultiplier;
