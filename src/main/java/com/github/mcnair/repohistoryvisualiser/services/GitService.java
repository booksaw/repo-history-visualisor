...
public class GitService {
    ...
    public RepositoryMetadata getRepositoryMetadata(String cloneURL, String branch, Git git, Settings settings) throws RepositoryTraverseException, IllegalBranchException {
        ...
        if (settings != null) {
            orderMilestoneData(branch, git, settings, commitCount);
        }

        return new RepositoryMetadata(cloneURL, branch, commitCount, settings);
    }

    private void orderMilestoneData(String branch, Git git, Settings settings, int commitCount) throws IllegalBranchException, RepositoryTraverseException {
        ...
        try {
            ...
            int id = commitCount;
            for (RevCommit commit : git.log().add(branchVar).call()) {
                var milestone = milestones.get(commit.getId().getName());
                if (milestone != null) {
        ...
        for (Map.Entry<String, Milestone> temp : milestones.entrySet()) {
            log.warn("Commit hash: " + temp.getValue().commitHash + " does not exist on git data");
